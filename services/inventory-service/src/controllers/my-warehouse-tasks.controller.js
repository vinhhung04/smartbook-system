  ...
  try {
    const [
      ...
      prisma.outbound_orders.findMany({
        where: { status: 'APPROVED', processed_by_user_id: null },
        select: {
          id: true,
      ...
      prisma.transfer_orders.findMany({
        where: { status: 'APPROVED', shipped_by_user_id: null },
        select: {
          id: true,
      ...
      prisma.transfer_orders.findMany({
        where: { status: 'READY_FOR_OUTBOUND', outbound_assigned_user_id: null },
        select: {
          id: true,
          transfer_number: true,
          status: true,
      ...
      // 5. Transfer receiving: IN_TRANSIT, chưa có người nhận
      prisma.transfer_orders.findMany({
        where: { status: 'IN_TRANSIT', received_by_user_id: null },
        select: {
          id: true,
          transfer_number: true,
          status: true,
