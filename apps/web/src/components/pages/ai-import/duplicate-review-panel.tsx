...
import { formatDuplicateClassification } from "./utils";
    ...
    <section className="space-y-3" aria-label={t("duplicate_intelligence.title")}>
      <div className="flex flex-wrap items-center justify-between gap-2">
        <div className="flex flex-wrap items-center gap-1.5 text-[13px] text-muted-foreground">
          {formatDuplicateClassification(review.classification)}
          <ConfidenceMeter value={review.similarityScore} tone="indigo" />
          {Math.round(review.similarityScore * 100)}%
        </div>
        <div className="flex flex-wrap gap-1.5">
          {review.classification === "EXACT_DUPLICATE" && review.candidates[0] ? <button type="button" onClick={() => onAction("LINK_EXISTING_VARIANT", review.candidates[0])} className="min-h-9 rounded-md border border-indigo-200 bg-indigo-50 px-3 text-[12px] font-semibold text-indigo-700 hover:bg-indigo-100 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-indigo-500/30 dark:border-indigo-500/20 dark:bg-indigo-500/10 dark:text-indigo-300">{t("duplicate_intelligence.link_variant")}</button> : null}
          {review.classification === "SAME_EDITION" && review.candidates[0] ? <button type="button" onClick={() => onAction("CREATE_VARIANT_FOR_EDITION", review.candidates[0])} className="min-h-9 rounded-md border border-indigo-200 bg-indigo-50 px-3 text-[12px] font-semibold text-indigo-700 hover:bg-indigo-100 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-indigo-500/30 dark:border-indigo-500/20 dark:bg-indigo-500/10 dark:text-indigo-300">{t("duplicate_intelligence.create_variant")}</button> : null}
          {review.classification === "SAME_WORK_DIFFERENT_EDITION" && review.candidates[0] ? <button type="button" onClick={() => onAction("CREATE_NEW_EDITION", review.candidates[0])} className="min-h-9 rounded-md border border-indigo-200 bg-indigo-50 px-3 text-[12px] font-semibold text-indigo-700 hover:bg-indigo-100 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-indigo-500/30 dark:border-indigo-500/20 dark:bg-indigo-500/10 dark:text-indigo-300">{t("duplicate_intelligence.create_edition")}</button> : null}
      ...
      {review.candidates.length ? <ul className="space-y-1.5" role="list">{review.candidates.slice(0, 3).map((candidate) => (
        <li key={candidate.bookId} className="flex flex-wrap items-center gap-1.5 rounded-md border border-border bg-muted/[0.1] px-3 py-2.5 text-[13px]">
          <span className="font-semibold text-foreground">{candidate.title}</span>
          · {formatDuplicateClassification(candidate.classification)}
          <ConfidenceMeter value={candidate.score} tone="indigo" />
