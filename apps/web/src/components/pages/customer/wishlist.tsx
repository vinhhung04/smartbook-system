    ...
    <div className="max-w-4xl mx-auto p-6 space-y-6">
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-[12px] text-slate-400 dark:text-slate-500">{items.length} sách trong danh sách</p>
        </div>
      </div>
      ...
      {loading ? (
        ...
        <div className="text-center py-16 bg-card rounded-xl border">
          ...
          <p className="text-[14px] text-muted-foreground" style={{ fontWeight: 550 }}>Chưa có sách yêu thích</p>
          <p className="text-[12px] text-slate-400 dark:text-slate-500 mt-1">Thêm sách vào danh sách từ trang Catalog</p>
        ...
        <div className="space-y-3">
          {items.map((item, i) => (
            <motion.div key={item.id} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.04 }}
              className="bg-card rounded-xl border p-4 flex items-center gap-4 hover:shadow-md transition-all">
              <div className="w-12 h-12 rounded-lg bg-gradient-to-br from-indigo-100 to-violet-100 dark:from-indigo-950/40 dark:to-violet-950/30 flex items-center justify-center shrink-0">
                <BookOpen className="w-6 h-6 text-indigo-500 dark:text-indigo-400" />
              </div>
              <div className="flex-1 min-w-0">
                <NavLink to={`/customer/books/${item.book_id}`} className="text-[14px] text-slate-800 dark:text-slate-200 hover:text-indigo-600 transition-colors truncate block" style={{ fontWeight: 600 }}>
                  {item.book_title || `Book ${item.book_id.slice(0, 8)}`}
                </NavLink>
                <p className="text-[12px] text-slate-400 dark:text-slate-500">{item.book_author || 'Unknown'} · Thêm {new Date(item.created_at).toLocaleDateString('vi-VN')}</p>
              </div>
