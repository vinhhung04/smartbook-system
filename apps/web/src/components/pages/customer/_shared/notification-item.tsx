...
export function NotificationItem({ item, onMarkedRead }: NotificationItemProps) {
  ...
  return (
    <div className={`rounded-[12px] border p-3.5 ${unread ? 'border-indigo-100 bg-indigo-50/40' : 'border-border bg-card'}`}>
      <div className="flex items-start gap-3">
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-center justify-between gap-2">
            <div className="text-[13px] text-foreground" style={{ fontWeight: 700 }}>{item.subject || item.template_code || 'Thông báo'}</div>
            <div className="flex items-center gap-1.5">
              ...
              {unread ? <span className="rounded-[8px] bg-indigo-100 px-2 py-0.5 text-[10px] uppercase text-indigo-700">Chưa đọc</span> : null}
            </div>
          </div>
          <div className="mt-1.5 text-[13px] text-slate-600 dark:text-slate-300">{item.body}</div>
