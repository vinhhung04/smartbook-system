import { createContext, useContext, useState, useCallback, useEffect, type ReactNode } from 'react';

export type Locale = 'vi' | 'en';

const translations: Record<Locale, Record<string, string>> = {
  vi: {
    // Sidebar group labels
    'sidebar.group.core': 'Vận hành',
    'sidebar.group.intelligence': 'Phân tích & AI',
    'sidebar.group.library': 'Thư viện',
    'sidebar.group.admin': 'Quản trị',
    'sidebar.group.today': 'Công việc hôm nay',
    'sidebar.group.warehouse': 'Vận hành kho',
    'sidebar.group.purchasing': 'Mua hàng & NCC',
    'sidebar.group.analytics': 'Báo cáo & AI',
    'sidebar.group.communication': 'Liên lạc nội bộ',
    // Sidebar items
    'sidebar.dashboard': 'Tổng quan',
    'sidebar.messages': 'Tin nhắn',
    'sidebar.my_tasks': 'Công việc kho của tôi',
    'sidebar.my_purchase_requests': 'Yêu cầu mua hàng của tôi',
    'sidebar.my_exception_reports': 'Báo cáo sự cố của tôi',
    'sidebar.staff_tasks': 'Giao việc nhân viên',
    'sidebar.catalog': 'Danh mục sách',
    'sidebar.inventory': 'Tồn kho',
    'sidebar.goods_receipts': 'Phiếu nhập kho',
    'sidebar.purchase_orders': 'Đơn đặt hàng',
    'sidebar.purchase_requests': 'Yêu cầu mua hàng',
    'sidebar.exception_reports': 'Báo cáo sự cố',
    'sidebar.supplier_deliveries': 'Giao hàng từ NCC',
    'sidebar.receiving_check': 'Kiểm đếm hàng nhận',
    'sidebar.putaway': 'Cất hàng vào kệ',
    'sidebar.receiving_putaway': 'Nhận & cất hàng',
    'sidebar.order_requests': 'Yêu cầu xuất kho',
    'sidebar.picking': 'Lấy hàng',
    'sidebar.packing': 'Đóng gói',
    'sidebar.outbound': 'Xuất kho',
    'sidebar.warehouses': 'Kho',
    'sidebar.shelves': 'Kệ',
    'sidebar.movements': 'Lịch sử kho',
    'sidebar.stock_audits': 'Kiểm kê kho',
    'sidebar.suppliers': 'Nhà cung cấp',
    'sidebar.ai_import': 'Nhập sách qua AI',
    'sidebar.receiving_smart': 'Nhập kho thông minh',
    'isbn_intelligence.title': 'Smart ISBN Intelligence',
    'isbn_intelligence.quality': 'Chất lượng metadata',
    'isbn_intelligence.processing': 'Xử lý',
    'isbn_intelligence.conflicts': 'mâu thuẫn',
    'isbn_intelligence.conflict_warning': 'Các nguồn có dữ liệu không đồng nhất; hãy kiểm tra trước khi lưu.',
    'isbn_intelligence.evidence': 'Bằng chứng theo trường',
    'isbn_intelligence.confirmed_by': 'Xác nhận bởi',
    'isbn_intelligence.sources': 'Trạng thái nguồn',
    'isbn_intelligence.coverage': 'Độ đầy đủ metadata',
    'isbn_intelligence.enriched_from': 'Bổ sung từ',
    'isbn_intelligence.not_found_reliable': 'Chưa tìm thấy từ nguồn đáng tin cậy',
    'isbn_intelligence.low_confidence': 'Độ tin cậy thấp — nên kiểm tra',
    'isbn_intelligence.missing_prefix': 'Thiếu',
    'isbn_intelligence.selected': 'Đã chọn',
    'isbn_intelligence.reason.multi_source': 'Đồng thuận nhiều nguồn',
    'isbn_intelligence.reason.single_source': '1 nguồn',
    'isbn_intelligence.reason.source_conflict': 'Mâu thuẫn nguồn',
    'isbn_intelligence.reason.minority_disagreement': 'Có ý kiến khác',
    'isbn_intelligence.reason.category_union': 'Gộp từ nhiều nguồn',
    'isbn_intelligence.reason.richest_description': 'Bản mô tả đầy đủ nhất',
    'isbn_intelligence.reason.llm_rejected': 'AI suy luận — chưa dùng do thiếu nguồn',
    'isbn_intelligence.method.api_structured': 'Dữ liệu có cấu trúc',
    'isbn_intelligence.method.json_ld': 'JSON-LD',
    'isbn_intelligence.method.dom': 'Trích xuất trang',
    'isbn_intelligence.method.snippet_regex': 'Đoạn trích tìm kiếm',
    'isbn_intelligence.method.llm_derived': 'AI suy luận',
    'isbn_intelligence.view_source': 'Xem nguồn',
    'metadata_reconciliation.title': 'Chuẩn hóa authority',
    'metadata_reconciliation.hint': 'So sánh dữ liệu nguồn, chuẩn hóa rule và authority catalog trước khi áp dụng.',
    'metadata_reconciliation.authors': 'Tác giả',
    'metadata_reconciliation.publisher': 'Nhà xuất bản',
    'metadata_reconciliation.categories': 'Thể loại',
    'metadata_reconciliation.warnings': 'cảnh báo chất lượng',
    'metadata_reconciliation.no_warnings': 'Không có cảnh báo',
    'metadata_reconciliation.new_entity': 'Entity mới cần duyệt',
    'metadata_reconciliation.accept': 'Chấp nhận',
    'metadata_reconciliation.reject': 'Từ chối',
    'metadata_reconciliation.create_entity': 'Tạo entity mới',
    'duplicate_intelligence.title': 'Kiểm tra trùng lặp & edition',
    'duplicate_intelligence.dismiss': 'Bỏ qua cảnh báo',
    'duplicate_intelligence.new_title': 'Đầu sách mới',
    'duplicate_intelligence.link_variant': 'Liên kết variant hiện có',
    'duplicate_intelligence.create_variant': 'Tạo variant mới',
    'duplicate_intelligence.create_edition': 'Tạo edition mới',
    'duplicate_intelligence.create_title': 'Tạo đầu sách mới',
    'sidebar.recommendations': 'Gợi ý sách',
    'sidebar.ai_reorder': 'Đề xuất & rủi ro AI',
    'sidebar.ai_assistant': 'Trợ lý AI',
    'sidebar.reports': 'Báo cáo',
    'sidebar.borrow': 'Mượn trả',
    'sidebar.borrow_customers': 'Bạn đọc mượn',
    'sidebar.reservations': 'Đặt trước',
    'sidebar.loans': 'Phiếu mượn',
    'sidebar.fines': 'Tiền phạt',
    'sidebar.membership_plans': 'Gói thành viên',
    'sidebar.users': 'Người dùng',
    'sidebar.roles': 'Vai trò',
    'sidebar.system_monitor': 'Giám sát hệ thống',
    'sidebar.audit_trail': 'Nhật ký hệ thống',
    'sidebar.scan_receive_cta': 'Ghi nhận hàng nhận',
    'sidebar.group.catalog': 'Danh mục sách',
    // Command palette (Ctrl/Cmd+K)
    'command_palette.title': 'Tìm kiếm và điều hướng',
    'command_palette.description': 'Gõ để tìm màn hình, dùng phím mũi tên để chọn và Enter để mở.',
    'command_palette.search_placeholder': 'Tìm màn hình, chức năng...',
    'command_palette.no_results': 'Không tìm thấy màn hình nào khớp',
    'command_palette.navigate': 'Di chuyển',
    'command_palette.open': 'Mở',
    // Topbar
    'topbar.search_placeholder': 'Tìm sách, đơn hàng, mã vạch...',
    'topbar.notifications': 'Thông báo',
    'topbar.mark_all_read': 'Đánh dấu tất cả đã đọc',
    'topbar.no_events': 'Chưa có sự kiện trực tiếp nào.',
    'topbar.events_will_appear': 'Các sự kiện sẽ xuất hiện ở đây khi phát sinh.',
    'topbar.connected': 'Trực tuyến — đang nhận cập nhật',
    'topbar.disconnected': 'Mất kết nối — đang kết nối lại...',
    'topbar.logout': 'Đăng xuất',
    'topbar.logged_out': 'Đã đăng xuất',
    'topbar.quick_scan': 'Quét nhanh',
    'topbar.toggle_dark_mode': 'Chuyển nền tối',
    'topbar.user_fallback': 'Người dùng',
    'topbar.event.loan_status_changed': 'Cập nhật mượn',
    'topbar.event.reservation_status_changed': 'Cập nhật đặt trước',
    'topbar.event.fine_created': 'Cập nhật phạt',
    'topbar.event.notification_new': 'Thông báo',
    // Breadcrumb labels
    'breadcrumb.dashboard': 'Tổng quan',
    'breadcrumb.catalog': 'Danh mục sách',
    'breadcrumb.inventory': 'Tồn kho',
    'breadcrumb.goods_receipts': 'Phiếu nhập kho',
    'breadcrumb.new_receipt': 'Phiếu nhập mới',
    'breadcrumb.putaway': 'Cất hàng',
    'breadcrumb.receiving_putaway': 'Nhận & cất hàng',
    'breadcrumb.order_requests': 'Yêu cầu xuất kho',
    'breadcrumb.picking': 'Lấy hàng',
    'breadcrumb.outbound': 'Xuất kho',
    'breadcrumb.warehouses': 'Kho',
    'breadcrumb.shelves': 'Kệ',
    'breadcrumb.movements': 'Lịch sử kho',
    'breadcrumb.ai_import': 'Nhập sách qua AI',
    'breadcrumb.recommendations': 'Gợi ý sách',
    'breadcrumb.ai_reorder': 'Đề xuất & rủi ro AI',
    'breadcrumb.reports': 'Báo cáo',
    'breadcrumb.borrow': 'Mượn trả',
    'breadcrumb.users': 'Người dùng',
    'breadcrumb.roles': 'Vai trò',
    'breadcrumb.book_detail': 'Chi tiết sách',
    'breadcrumb.order_detail': 'Chi tiết đơn',
    'breadcrumb.purchase_order_detail': 'Chi tiết đơn mua',
    'breadcrumb.new_purchase_order': 'Đơn mua mới',
    'breadcrumb.putaway_detail': 'Chi tiết cất hàng',
    'breadcrumb.stock_audit_detail': 'Chi tiết kiểm kê',
    'breadcrumb.loan_detail': 'Chi tiết phiếu mượn',
    'breadcrumb.forbidden': 'Không có quyền truy cập',
    'breadcrumb.receiving_smart': 'Nhập hàng thông minh (AI)',
    'breadcrumb.transfer_receiving': 'Nhận hàng chuyển kho',
    // Customer nav (giữ tương thích keys cũ)
    'nav.dashboard': 'Trang chủ',
    'nav.books': 'Danh mục sách',
    'nav.loans': 'Phiếu mượn',
    'nav.reservations': 'Đặt trước',
    'nav.wishlist': 'Yêu thích',
    'nav.reading_analytics': 'Thống kê đọc',
    'nav.membership': 'Thẻ thành viên',
    'nav.fines': 'Phạt',
    'nav.notifications': 'Thông báo',
    'nav.profile': 'Hồ sơ',
    // Common
    'common.loading': 'Đang tải...',
    'common.refresh': 'Làm mới',
    'common.search': 'Tìm kiếm...',
    'common.save': 'Lưu',
    'common.cancel': 'Hủy',
    'common.delete': 'Xóa',
    'common.edit': 'Sửa',
    'common.back': 'Quay lại',
    'common.confirm': 'Xác nhận',
    'common.no_data': 'Chưa có dữ liệu',
    'common.error': 'Đã xảy ra lỗi',
    'common.success': 'Thành công',
    'common.all': 'Tất cả',
    'common.available': 'Còn hàng',
    'common.out_of_stock': 'Hết hàng',
    'catalog.title': 'Danh mục sách',
    'catalog.subtitle': 'Khám phá và đặt mượn sách',
    'catalog.search_placeholder': 'Tìm theo tên, tác giả, ISBN...',
    'catalog.all_categories': 'Tất cả thể loại',
    'catalog.all_authors': 'Tất cả tác giả',
    'catalog.all_availability': 'Tất cả tình trạng',
    'catalog.reserve': 'Đặt mượn',
    'catalog.view_detail': 'Xem chi tiết',
    'loan.status.BORROWED': 'Đang mượn',
    'loan.status.RETURNED': 'Đã trả',
    'loan.status.OVERDUE': 'Quá hạn',
    'loan.status.CANCELLED': 'Đã hủy',
    'dashboard.welcome': 'Xin chào',
    'dashboard.overview': 'Tổng quan hoạt động',
    'wishlist.title': 'Sách yêu thích',
    'wishlist.empty': 'Chưa có sách yêu thích',
    'wishlist.add': 'Thêm vào yêu thích',
    'wishlist.remove': 'Xóa khỏi yêu thích',
    'reading.title': 'Thống kê đọc sách',
    'reading.subtitle': 'Hành trình đọc sách của bạn',
    'reading.total_books': 'Tổng sách đã mượn',
    'reading.avg_days': 'Thời gian mượn TB',
    'reading.streak': 'Streak liên tục',
    'reading.achievements': 'Thành tựu',
    'membership.qr_title': 'Mã QR thẻ thành viên',
    'membership.qr_subtitle': 'Xuất trình khi đến thư viện',
  },
  en: {
    'sidebar.group.core': 'Operations',
    'sidebar.group.intelligence': 'Analytics & AI',
    'sidebar.group.library': 'Library',
    'sidebar.group.admin': 'Administration',
    'sidebar.group.today': "Today's Work",
    'sidebar.group.warehouse': 'Warehouse Ops',
    'sidebar.group.purchasing': 'Purchasing',
    'sidebar.group.analytics': 'Reports & AI',
    'sidebar.group.communication': 'Internal Messaging',
    'sidebar.dashboard': 'Dashboard',
    'sidebar.messages': 'Messages',
    'sidebar.my_tasks': 'My Warehouse Tasks',
    'sidebar.my_purchase_requests': 'My Purchase Requests',
    'sidebar.my_exception_reports': 'My Exception Reports',
    'sidebar.staff_tasks': 'Assign Staff Tasks',
    'sidebar.catalog': 'Catalog',
    'sidebar.inventory': 'Inventory',
    'sidebar.goods_receipts': 'Goods Receipts',
    'sidebar.purchase_orders': 'Purchase Orders',
    'sidebar.purchase_requests': 'Purchase Requests',
    'sidebar.exception_reports': 'Exception Reports',
    'sidebar.supplier_deliveries': 'Supplier Deliveries',
    'sidebar.receiving_check': 'Receiving Check',
    'sidebar.putaway': 'Putaway',
    'sidebar.receiving_putaway': 'Receiving Putaway',
    'sidebar.order_requests': 'Order Requests',
    'sidebar.picking': 'Picking',
    'sidebar.packing': 'Packing',
    'sidebar.outbound': 'Outbound',
    'sidebar.warehouses': 'Warehouses',
    'sidebar.shelves': 'Shelves',
    'sidebar.movements': 'Stock Movements',
    'sidebar.stock_audits': 'Stock Audit',
    'sidebar.suppliers': 'Suppliers',
    'sidebar.ai_import': 'AI Import',
    'sidebar.receiving_smart': 'Smart Receiving',
    'isbn_intelligence.title': 'Smart ISBN Intelligence',
    'isbn_intelligence.quality': 'Metadata quality',
    'isbn_intelligence.processing': 'Processing',
    'isbn_intelligence.conflicts': 'conflicts',
    'isbn_intelligence.conflict_warning': 'Sources disagree; review the values before saving.',
    'isbn_intelligence.evidence': 'Field evidence',
    'isbn_intelligence.confirmed_by': 'Confirmed by',
    'isbn_intelligence.sources': 'Source status',
    'isbn_intelligence.coverage': 'Metadata completeness',
    'isbn_intelligence.enriched_from': 'Added from',
    'isbn_intelligence.not_found_reliable': 'Not found in a reliable source',
    'isbn_intelligence.low_confidence': 'Low confidence — please verify',
    'isbn_intelligence.missing_prefix': 'Missing',
    'isbn_intelligence.selected': 'Selected',
    'isbn_intelligence.reason.multi_source': 'Multiple sources agree',
    'isbn_intelligence.reason.single_source': '1 source',
    'isbn_intelligence.reason.source_conflict': 'Sources conflict',
    'isbn_intelligence.reason.minority_disagreement': 'One source disagrees',
    'isbn_intelligence.reason.category_union': 'Merged from multiple sources',
    'isbn_intelligence.reason.richest_description': 'Most complete description',
    'isbn_intelligence.reason.llm_rejected': 'AI-inferred — not used, no source backing',
    'isbn_intelligence.method.api_structured': 'Structured data',
    'isbn_intelligence.method.json_ld': 'JSON-LD',
    'isbn_intelligence.method.dom': 'Page extraction',
    'isbn_intelligence.method.snippet_regex': 'Search snippet',
    'isbn_intelligence.method.llm_derived': 'AI-inferred',
    'isbn_intelligence.view_source': 'View source',
    'metadata_reconciliation.title': 'Authority normalization',
    'metadata_reconciliation.hint': 'Compare source data, deterministic normalization, and catalog authority before applying it.',
    'metadata_reconciliation.authors': 'Authors',
    'metadata_reconciliation.publisher': 'Publisher',
    'metadata_reconciliation.categories': 'Categories',
    'metadata_reconciliation.warnings': 'quality warnings',
    'metadata_reconciliation.no_warnings': 'No warnings',
    'metadata_reconciliation.new_entity': 'New entity needs review',
    'metadata_reconciliation.accept': 'Accept',
    'metadata_reconciliation.reject': 'Reject',
    'metadata_reconciliation.create_entity': 'Create new entity',
    'duplicate_intelligence.title': 'Duplicate & edition check',
    'duplicate_intelligence.dismiss': 'Dismiss warning',
    'duplicate_intelligence.new_title': 'New title',
    'duplicate_intelligence.link_variant': 'Link existing variant',
    'duplicate_intelligence.create_variant': 'Create new variant',
    'duplicate_intelligence.create_edition': 'Create new edition',
    'duplicate_intelligence.create_title': 'Create new title',
    'sidebar.recommendations': 'Recommendations',
    'sidebar.ai_reorder': 'AI Suggestions & Risk',
    'sidebar.ai_assistant': 'AI Assistant',
    'sidebar.reports': 'Reports',
    'sidebar.borrow': 'Borrow',
    'sidebar.borrow_customers': 'Borrow Customers',
    'sidebar.reservations': 'Reservations',
    'sidebar.loans': 'Loans',
    'sidebar.fines': 'Fines',
    'sidebar.membership_plans': 'Membership Plans',
    'sidebar.users': 'Users',
    'sidebar.roles': 'Roles',
    'sidebar.system_monitor': 'System Monitor',
    'sidebar.audit_trail': 'Audit Trail',
    'sidebar.scan_receive_cta': 'Scan & Receive',
    'sidebar.group.catalog': 'Book Catalog',
    'command_palette.title': 'Search and navigate',
    'command_palette.description': 'Type to search screens, use arrow keys to select, and Enter to open.',
    'command_palette.search_placeholder': 'Search screens, features...',
    'command_palette.no_results': 'No matching screen found',
    'command_palette.navigate': 'Navigate',
    'command_palette.open': 'Open',
    'topbar.search_placeholder': 'Search books, orders, barcodes...',
    'topbar.notifications': 'Notifications',
    'topbar.mark_all_read': 'Mark all read',
    'topbar.no_events': 'No real-time events yet.',
    'topbar.events_will_appear': 'Events will appear here as they happen.',
    'topbar.connected': 'Live — receiving real-time updates',
    'topbar.disconnected': 'Offline — reconnecting...',
    'topbar.logout': 'Logout',
    'topbar.logged_out': 'Logged out',
    'topbar.quick_scan': 'Quick scan',
    'topbar.toggle_dark_mode': 'Toggle dark mode',
    'topbar.user_fallback': 'User',
    'topbar.event.loan_status_changed': 'Loan update',
    'topbar.event.reservation_status_changed': 'Reservation update',
    'topbar.event.fine_created': 'Fine update',
    'topbar.event.notification_new': 'Notification',
    'breadcrumb.dashboard': 'Dashboard',
    'breadcrumb.catalog': 'Catalog',
    'breadcrumb.inventory': 'Inventory',
    'breadcrumb.goods_receipts': 'Goods Receipts',
    'breadcrumb.new_receipt': 'New Receipt',
    'breadcrumb.putaway': 'Putaway',
    'breadcrumb.receiving_putaway': 'Receiving Putaway',
    'breadcrumb.order_requests': 'Order Requests',
    'breadcrumb.picking': 'Picking',
    'breadcrumb.outbound': 'Outbound',
    'breadcrumb.warehouses': 'Warehouses',
    'breadcrumb.shelves': 'Shelves',
    'breadcrumb.movements': 'Stock Movements',
    'breadcrumb.ai_import': 'AI Import',
    'breadcrumb.recommendations': 'Recommendations',
    'breadcrumb.ai_reorder': 'AI Suggestions & Risk',
    'breadcrumb.reports': 'Reports',
    'breadcrumb.borrow': 'Borrow',
    'breadcrumb.users': 'Users',
    'breadcrumb.roles': 'Roles',
    'breadcrumb.book_detail': 'Book Detail',
    'breadcrumb.order_detail': 'Order Detail',
    'breadcrumb.purchase_order_detail': 'Purchase Order Detail',
    'breadcrumb.new_purchase_order': 'New Purchase Order',
    'breadcrumb.putaway_detail': 'Putaway Detail',
    'breadcrumb.stock_audit_detail': 'Stock Audit Detail',
    'breadcrumb.loan_detail': 'Loan Detail',
    'breadcrumb.forbidden': 'Forbidden',
    'breadcrumb.receiving_smart': 'Smart Receiving (AI)',
    'breadcrumb.transfer_receiving': 'Transfer Receiving',
    'nav.dashboard': 'Dashboard',
    'nav.books': 'Browse Books',
    'nav.loans': 'My Loans',
    'nav.reservations': 'My Reservations',
    'nav.wishlist': 'Wishlist',
    'nav.reading_analytics': 'Reading Analytics',
    'nav.membership': 'My Membership',
    'nav.fines': 'My Fines',
    'nav.notifications': 'Notifications',
    'nav.profile': 'My Profile',
    'common.loading': 'Loading...',
    'common.refresh': 'Refresh',
    'common.search': 'Search...',
    'common.save': 'Save',
    'common.cancel': 'Cancel',
    'common.delete': 'Delete',
    'common.edit': 'Edit',
    'common.back': 'Back',
    'common.confirm': 'Confirm',
    'common.no_data': 'No data yet',
    'common.error': 'An error occurred',
    'common.success': 'Success',
    'common.all': 'All',
    'common.available': 'Available',
    'common.out_of_stock': 'Out of stock',
    'catalog.title': 'Book Catalog',
    'catalog.subtitle': 'Discover and reserve books',
    'catalog.search_placeholder': 'Search by title, author, ISBN...',
    'catalog.all_categories': 'All categories',
    'catalog.all_authors': 'All authors',
    'catalog.all_availability': 'All availability',
    'catalog.reserve': 'Reserve',
    'catalog.view_detail': 'View detail',
    'loan.status.BORROWED': 'Borrowed',
    'loan.status.RETURNED': 'Returned',
    'loan.status.OVERDUE': 'Overdue',
    'loan.status.CANCELLED': 'Cancelled',
    'dashboard.welcome': 'Welcome',
    'dashboard.overview': 'Activity overview',
    'wishlist.title': 'Wishlist',
    'wishlist.empty': 'No books in wishlist',
    'wishlist.add': 'Add to wishlist',
    'wishlist.remove': 'Remove from wishlist',
    'reading.title': 'Reading Analytics',
    'reading.subtitle': 'Your reading journey',
    'reading.total_books': 'Total books borrowed',
    'reading.avg_days': 'Average borrow time',
    'reading.streak': 'Streak',
    'reading.achievements': 'Achievements',
    'membership.qr_title': 'Membership QR Code',
    'membership.qr_subtitle': 'Show at library entrance',
  },
};

interface I18nContextValue {
  locale: Locale;
  setLocale: (l: Locale) => void;
  t: (key: string) => string;
}

const I18nContext = createContext<I18nContextValue>({
  locale: 'vi',
  setLocale: () => {},
  t: (key) => key,
});

export function I18nProvider({ children }: { children: ReactNode }) {
  const [locale, setLocaleState] = useState<Locale>(() => {
    const saved = localStorage.getItem('smartbook-locale') as Locale | null;
    return saved || 'vi';
  });

  useEffect(() => {
    if (typeof document !== 'undefined') {
      document.documentElement.lang = locale;
    }
  }, [locale]);

  const setLocale = useCallback((l: Locale) => {
    setLocaleState(l);
    localStorage.setItem('smartbook-locale', l);
  }, []);

  const t = useCallback((key: string): string => {
    return translations[locale]?.[key] || translations.vi[key] || key;
  }, [locale]);

  return (
    <I18nContext.Provider value={{ locale, setLocale, t }}>
      {children}
    </I18nContext.Provider>
  );
}

export function useI18n() {
  return useContext(I18nContext);
}

export function LanguageToggle({ className }: { className?: string }) {
  const { locale, setLocale } = useI18n();
  return (
    <button
      onClick={() => setLocale(locale === 'vi' ? 'en' : 'vi')}
      className={`inline-flex items-center justify-center gap-1.5 px-2.5 h-8 rounded-[8px] text-[11px] text-slate-500 hover:bg-slate-100 dark:hover:bg-slate-800 dark:text-slate-400 transition-all duration-140 ${className || ''}`}
      style={{ fontWeight: 600 }}
      title={locale === 'vi' ? 'Switch to English' : 'Chuyển sang Tiếng Việt'}
      aria-label={locale === 'vi' ? 'Switch to English' : 'Chuyển sang Tiếng Việt'}
    >
      {locale === 'vi' ? 'VI' : 'EN'}
    </button>
  );
}
